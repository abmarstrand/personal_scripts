...
builds interval trees for rapid lookup, iterates over a BAM file (using pysam)
to classify each read into “spliced”, “unspliced” or “ambiguous” for each gene,
and writes the counts to a loom file with three layers.
...
Note:
   • The BAM file must be coordinate-sorted by mapping position or, if not, you can let this script sort it.
   • The script now accepts the --already_sorted flag to indicate that the provided BAM is already sorted.
   • The read-classification algorithm mimics velocyto’s default behavior:
     a read is classified as “spliced” if all its aligned blocks fall within annotated exons,
...
def create_loom_from_bam_gtf(
    ...
    *,
    ...
    Parameters:
      ...
      already_sorted (bool): If True, assume the provided BAM file is already sorted.
      ...
      loom_numeric_dtype (str): Numeric dtype for loom layers (e.g., "uint32").
    ...
    logging.info(f"Parsed {len(genes)} genes.")
    
    logging.info("Building interval trees...")
    trees = build_interval_trees(genes)
    
    # Determine BAM file to use.
    if already_sorted:
        bam_sorted = bam_file
    ...
    bam_in.close()
    logging.info(f"Total processed reads: {n_processed}")
    
    # Collect all cell barcodes
    all_gene_ids = sorted(genes.keys())
    all_cells = set()
    for d in (spliced_counts, unspliced_counts, ambiguous_counts):
        for gene in d:
            all_cells.update(d[gene].keys())
    all_cells = sorted(all_cells)
    
    n_genes = len(all_gene_ids)
    n_cells = len(all_cells)
    logging.info(f"Building matrices: {n_genes} genes x {n_cells} cells")
    
    def build_matrix(counts_dict):
        mat = np.zeros((n_genes, n_cells), dtype=np.uint32)
        cell_to_ix = {cell: i for i, cell in enumerate(all_cells)}
    ...
    ca = {"CellID": np.array(all_cells, dtype=str)}
    ...
    loompy.create(
        ...
        file_attrs={
            ...
            "notes": "Counts generated by a stand-alone script; classification mimics velocyto Default logic."
        }
    )
